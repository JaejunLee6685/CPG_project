...
ptex = pd.read_csv(snakemake.input['pext_value'],compression='gzip', sep='\t', low_memory=False)

delmis = pd.read_csv(snakemake.input['need2check'],compression='gzip', sep='\t', low_memory=False)
...
ptex_delmis.to_csv(snakemake.output['pext_combine'],sep='\t', compression='gzip', index=False)
