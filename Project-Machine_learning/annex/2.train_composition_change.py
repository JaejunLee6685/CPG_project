...
# selection of train composition
selection_portion = float(sys.argv[1]) # import with sys.argv 
# range from 0.1 to 1.0 -> use selection_proprotion for file name
...
def process_iteration(i):
    ...
    selected_cpg = np.random.choice(cpg_list, int(round(len(cpg_list)*selection_portion)), replace=False ) # cpg selection with proportion
    selected_gene_other = np.random.choice(other_list, len(selected_cpg), replace=False)
    fin_gene = np.concatenate((selected_cpg,selected_gene_other))
...
FIN_GENE.to_csv('Result_machine_learning/%s_.ALL.permutation.%s.portion.%s.tsv' %(name, (first_permu*second_repeat), selection_portion), sep='\t')

# Machine learning performance save 
SCORE.to_csv('Result_machine_learning/%s.ALL.score.%s.portion.%s.tsv'%(name, (first_permu*second_repeat), selection_portion) , sep='\t')

# TPR / FPR save after each iteration steps
AUC_val.to_csv('Result_machine_learning/%s.ALL.TPR_FPR.score.%s.portion.%s.tsv'%(name, (first_permu*second_repeat), selection_portion) , sep='\t')